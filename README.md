...
```
# code used to install taskweaver, adapted for bioweaver
git clone https://github.com/merckey/bioweaver.git
cd bioweaver

conda create -n bioweaver python=3.10 -y
conda activate bioweaver


# install single cell library
# Bio-informatics stack (use conda-forge for safety with C++ dependecies)
conda install -c conda-forge scanpy python-igraph leidenalg networkx matplotlib seaborn -y
conda install pygraphviz

# Web UI stack (Chainlit is required for the playground UI)
# pip install chainlit
pip install -U "chainlit<1.1.300" # installed 1.1.202
# the default seting in requirement "pydantic>=2.8.2" not work for chainlit
pip install -U "pydantic==2.8.2"

# Install 
pip install -r requirements.txt

# config api
vi project/taskweaver_config.json	
```
...
2. Open chatbot in web browser ```http://localhost:8000/```

Example of the user query `perform clustering analysis using default parameter for the single cell data lcoated in /project/lji226_uksr/taskweaver/project/data/processed_scRNA.h5ad`.

3. Key folders
```
# python plugins
projects/
# data for testing single cell workflow
project/data/
