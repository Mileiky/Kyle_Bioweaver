...
class Session:
    ...
    def __init__(
        ...
        self.memory_path = os.path.join(self.workspace, f"{self.session_id}_memory.yaml")
        self._load_memory_if_exists()
    ...
    def _load_memory_if_exists(self) -> None:
        """
        Restore persisted conversation memory for an existing session if available.
        """
        if not os.path.exists(self.memory_path):
            return

        self.memory.from_yaml(self.session_id, self.memory_path)
